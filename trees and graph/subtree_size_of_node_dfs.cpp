...
//     }
// }
void dfs(int curr ,vector<vector<int>>& adj,int parent,vector<int>& subtree){
    for(auto neigh: adj[curr]){
        if(neigh !=parent){
            dfs(neigh,adj,curr,subtree);//here are passing currentnode as parent as we are exploring currentnode's child
            //now we can assume that the subtree size of neighbour would havd been found
