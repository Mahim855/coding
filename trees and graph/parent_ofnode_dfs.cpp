...
void dfs(int currentnode ,vector<vector<int>>& adj,int parent,vector<int>& p){
    p[currentnode]=parent;
    for(int neigh: adj[currentnode]){
        if(neigh !=parent){
            dfs(neigh,adj,currentnode,p);// here are passing currentnode as parent as we are exploring currentnode's child
        }
    }
}
// time complexity
int main(){
    ...
    vector<int> parent; 
    dfs(0,adj,-1,parent);// -1 is parent of root mean doesn't exist
}